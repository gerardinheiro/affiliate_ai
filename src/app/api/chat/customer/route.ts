...
Seja breve e direto nas respostas(máximo 3 parágrafos).`
    ...
    // Note: req.json() can only be called once. If an error occurs before it's parsed,
    // or if it's already parsed, we need to handle it.
    // For simplicity, we'll assume `body` is available or re-parse if needed.
    // In a real-world scenario, you might clone the request or store body earlier.
    let lastUserMessage = "";
    try {
        const body = await req.json(); // This might fail if already consumed or invalid JSON
        lastUserMessage = body.messages[body.messages.length - 1]?.content || "";
    } catch (parseError) {
        console.error("Failed to re-parse request body in error handler:", parseError);
        // Fallback to a default message if we can't get the last user message
        lastUserMessage = ""; 
    }
