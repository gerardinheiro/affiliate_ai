...
export async function generateCopyAction(productName: string, description?: string) {
    try {
        ...
        console.error("Error in generateCopyAction:", error)
...
export async function scrapeProductAction(url: string) {
    try {
        ...
        console.error("Error in scrapeProductAction:", error)
        // Return a fallback instead of throwing to avoid 500
        return {
            title: "Produto Exemplo (Erro)",
            description: "Não foi possível extrair a descrição automaticamente.",
            price: "R$ 0,00",
            imageUrl: "https://placehold.co/600x400/png?text=Erro+ao+Carregar",
            platform: "Erro"
        }
    }
}
